...
//this program solves travelling salesman problem with pruning algorithm
public class TravelPrune
{
  ...
  //Constructor of TravelPrune class
  public TravelPrune(int vertices, int[][] edges, PrintWriter PWOut)
  {
  ...
  public void timedSolve()
  {
    ...
    while (!openList.isEmpty())						//while openList still has paths
    {
      ...
      {
		  if (path.cost() < minimumCost)			//if partial path cost is less 
		  {											//than minimum cycle cost
			LinkedList<Path> children = generate(path);	//generate children of path

			for (int i = 0; i < children.size(); i++)
			  openList.addFirst(children.get(i));	 		//add children to beginning of list
		  }
      }
    }
